...
async function demandeInfosSurpartieAuBackend() {
   ...
   try {
         ...
         const debut = urlActualiserDelais.indexOf("envoi_info_frontend");
         const taille = "envoi_info_frontend".length;
         tailleUrlAvantDonnees = debut + taille;
         console.log("tailleUrlAvantDonnees: " + tailleUrlAvantDonnees);
         urlActualiserDelais = urlActualiserDelais.slice(tailleUrlAvantDonnees);

         const [partieId, typeInfo, numeroEpreuve, epreuveEtat, codeConnexion] = urlActualiserDelais.split("/");
         console.log("URL à contacter: "+ urlActualiserDelais) ;
         console.log("Analyse de l'URL: " + partieId + "," + typeInfo + "," + numeroEpreuve + "," + epreuveEtat);
