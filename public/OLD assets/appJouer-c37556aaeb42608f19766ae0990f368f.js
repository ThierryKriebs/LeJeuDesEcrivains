...
//        delayEpreuve = data;
//        delayEpreuveMilli = delayEpreuve * 1000; //Pour l'avoir en millisecondes
...
async function recupererDureeRestanteEpreuve() {
   ...
   try {
         ...
         if (!response.ok) {
            throw new Error(`Erreur lors de la réception de la durée restante de l'épreuve: ${response.status}`);
         }

         const data = await response.json();
         console.log("Réponse reçue :", data);
         console.log("Timeout dans fonction: " + timeoutId);

         if (Number.isInteger(data.dureeRestante)) {
            delayEpreuve = data.dureeRestante;
            delayEpreuveMilli = delayEpreuve * 1000; //Pour l'avoir en millisecondes
            ...
            console.log("Nouveau timeout dans fonction: " + timeoutId);
         }

         else {
            console.log("Erreur. Lors de la demande de la durée restante de l'épreuve, le serveur a retourné l'erreur suivante: ", data.dureeRestante);
         }
         

   } catch (error) {
            console.error("Erreur lors de la demande au serveur de la durée restante de l'épreuve: ", error);
   }
}
