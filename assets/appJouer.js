...
async function demandeInfosSurpartieAuBackend() {
   ...
   try {
         ...
         console.log("URL A contacter: " + urlContacterBackend);
        
         let urlContacterBackend2 = urlContacterBackend.slice(tailleUrlAvantDonnees);
         ...
         if (numeroEpreuve != data.numeroEpreuve || epreuveEtat != data.epreuveEtat || typeInfo != data.typeInfo || FlagRedacsTerminees != data.FlagRedacsTerminees || FlagNotationsTerminees != data.FlagNotationsTerminees) {
            ...
            console.log("Les données ont changées => Redirection sur la route jouer.");
            ...
            if (data.erreur !== "")
            {
               console.log("ERREUR, lors de la demande des infos au serveur. => Redirection sur route jouer. Le serveur a retourné l'erreur suivante: ", data.erreur);
